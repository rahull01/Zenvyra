/**
 * Canonical pricing plan definitions. Shared between frontend modules
 * that need to render plan info without depending on the Next.js app
 * tree. Mirrors `frontend/lib/pricing-plans.ts`. If you change one,
 * change the other.
 *
 * The `id` and `backendPlanType` values MUST match the backend
 * `PlanType` enum (`FREE`, `GROWTH`, `PRO`, `AGENCY`).
 */
...
export function planName(planId?: string): string {
  return PRICING_PLANS.find((p) => p.id === planId)?.name || "Free";
}
