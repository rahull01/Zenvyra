...
class IntegrationControllerTest {
    ...
    void downloadWordPressPlugin_shouldReturnValidZipFile() {
        ...
        request.setServerName("api.complianceai.pro");
        request.setServerPort(443);

        ResponseEntity<byte[]> response = controller.downloadWordPressPlugin("active-banner-123", request);

        assertEquals(200, response.getStatusCode().value());
        assertEquals("attachment; filename=\"complianceai-pro.zip\"", response.getHeaders().getFirst("Content-Disposition"));
    ...
    void getShopifyPixelCode_shouldReturnValidJavascriptBundle() {
        ...
        assertNotNull(body);
        assertTrue(body.contains("Shopify Custom Pixel for ComplianceAI Pro"));
    ...
    void downloadGtmTemplate_shouldReturnValidGtmContainerJson() {
        ...
        assertEquals(200, response.getStatusCode().value());
        assertEquals("attachment; filename=\"gtm-complianceai-template.json\"", response.getHeaders().getFirst("Content-Disposition"));
        assertEquals("application/json", response.getHeaders().getContentType().toString());
        String body = response.getBody();
        assertNotNull(body);
        assertTrue(body.contains("ComplianceAI GTM Template"));
        assertTrue(body.contains("ComplianceAI Banner Loader"));
    }
}
