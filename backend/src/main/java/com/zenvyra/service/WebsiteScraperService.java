...
public class WebsiteScraperService {
    ...
    public String extractHostDomain(String urlStr) {
        ...
        try {
            ...
            log.warn("Failed to extract host domain from {}: {}", LogSanitizer.url(urlStr), LogSanitizer.exception(e));
        }
        return null;
    }
    ...
    public Set<String> scrapeTrackingDomains(String targetUrl) throws IOException {
        ...
        log.info("Scraping third-party tracking scripts/iframes from {}", LogSanitizer.url(targetUrl));
        Document doc = Jsoup.connect(targetUrl)
                .userAgent("Zenvyra-Bot/1.0")
                .timeout(10000)
        ...
        log.info("Found {} unique external tracking domains for {}", externalDomains.size(), LogSanitizer.url(targetUrl));
        return externalDomains;
    }
