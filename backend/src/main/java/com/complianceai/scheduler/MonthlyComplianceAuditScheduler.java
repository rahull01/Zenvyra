...
import com.complianceai.service.OpenAiService;
import com.complianceai.service.PolicyService;
import com.complianceai.service.TrackerClassificationService;
import com.complianceai.service.WebsiteScraperService;
...
public class MonthlyComplianceAuditScheduler {
    ...
    private void auditSubscription(Subscription subscription) {
        ...
        for (Website website : websites) {
            try {
                ...
                log.error("Monthly audit failed for website {} owned by {}", website.getUrl(), user.getEmail(), e);
            }
        }
    }
    ...
    private void auditWebsite(User user, Website website) throws Exception {
        ...
        if (newDomains.isEmpty()) {
            log.info("No new tracker domains found for {}", website.getUrl());
            return;
        }

        log.info("Detected {} new tracker domains for {}: {}", newDomains.size(), website.getUrl(), newDomains);
    ...
    private void regenerateAffectedPolicies(
        ...
        for (Policy policy : affectedPolicies) {
            try {
                ...
                log.error("Failed to regenerate policy {} after monthly tracker audit", policy.getId(), e);
            }
        }
    }
