...
export const Footer = () => {
  return (
    <footer className="bg-slate-50 text-slate-950 pt-16 pb-8 px-6 border-t border-slate-200">
      <div className="max-w-[1400px] mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-12 mb-12">
          {/* Brand Column */}
          <div className="lg:col-span-2 space-y-6">
            <Link href="/" className="flex items-center gap-2">
              <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-orange-500 to-orange-700 flex items-center justify-center shadow-sm shadow-orange-200/40">
                <Shield className="w-6 h-6 text-white" />
              </div>
              <span className="font-bold text-2xl tracking-tight text-slate-950">ComplianceAI Pro</span>
            </Link>
            ...
            <div className="flex gap-3">
              {[Twitter, Linkedin, Facebook].map((Icon, i) => (
                <Link
                  key={i}
                  href="#"
                  className="p-2.5 rounded-lg bg-slate-100 text-orange-600 hover:bg-orange-100 hover:text-orange-800 transition-colors"
                >
          ...
          <div className="space-y-6">
            ...
            <ul className="space-y-3">
              {columns[0].links.map((link) => (
                <li key={link.name}>
                  <Link href={link.href} className="text-sm text-slate-600 hover:text-orange-600 transition-colors">
                    {link.name}
                  </Link>
                </li>
              ))}
        ...
        <div className="pt-8 border-t border-slate-200 flex flex-col md:flex-row justify-between items-center gap-4 text-sm">
          <p className="text-slate-500">© {new Date().getFullYear()} ComplianceAI Pro. All rights reserved.</p>
        </div>
